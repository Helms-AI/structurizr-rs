...
### Validate DSL
```bash
cargo run -- validate workspaces/large/enterprise-healthcare/workspace.dsl
cargo run -- validate workspaces/large/manufacturing-iot/workspace.dsl
```

### Render Diagrams
```bash
cargo run -- render --workspace workspaces/large/enterprise-healthcare/workspace.dsl --output ./output/healthcare
cargo run -- render --workspace workspaces/large/manufacturing-iot/workspace.dsl --output ./output/manufacturing
```

### Export to Other Formats
```bash
# JSON export
cargo run -- export --workspace workspaces/large/enterprise-healthcare/workspace.dsl --format json > healthcare.json

# PlantUML export
cargo run -- export --workspace workspaces/large/manufacturing-iot/workspace.dsl --format plantuml > manufacturing.puml
```

### Start Web Server
```bash
cargo run -- serve --workspace workspaces/large/enterprise-healthcare/workspace.dsl --port 8080
```
