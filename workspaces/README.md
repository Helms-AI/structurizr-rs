# structurizr-rs Workspaces

A comprehensive suite of enterprise-level architecture workspaces demonstrating the Structurizr DSL capabilities across different complexity levels and industry domains.
...
cargo run -- serve --data-dir workspaces/medium/ecommerce-platform --port 8080

# Validate a workspace
cargo run -- validate workspaces/small/startup-saas/workspace.dsl

# Export to different formats
cargo run -- export --workspace workspaces/large/enterprise-healthcare/workspace.dsl --format plantuml
```

---

## Workspaces by Complexity
...
# Render to SVG
cargo run -- render --workspace workspaces/medium/fintech-payments/workspace.dsl --output ./output

# Export formats
cargo run -- export --workspace workspaces/large/manufacturing-iot/workspace.dsl --format json
cargo run -- export --workspace workspaces/large/manufacturing-iot/workspace.dsl --format plantuml
cargo run -- export --workspace workspaces/large/manufacturing-iot/workspace.dsl --format mermaid
```
