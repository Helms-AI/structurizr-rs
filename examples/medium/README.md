...
```bash
cargo run -- validate examples/medium/ecommerce-platform/workspace.dsl
```

To render an example:
```bash
cargo run -- render --workspace examples/medium/ecommerce-platform/workspace.dsl --output ./output
```

To serve an example:
```bash
cargo run -- serve --workspace examples/medium/ecommerce-platform/workspace.dsl --port 8080
```
...
- Examine the simple examples in `/examples/simple/` for basic concepts
- Review the complex examples in `/examples/complex/` for large-scale systems
